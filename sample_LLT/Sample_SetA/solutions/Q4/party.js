...
const app = Vue.createApp( {
    ...
    methods: {
        ...
        add_to_directory() {
            ...
            // note: ignore the regex. Out of scope
            // let email_array = this.new_emails.split(/(?:,| )+/)
            let email_array = this.new_emails.split(", ") // this is fine
            ...
            for( email_item of email_array ) {
                ...
                if( email_item.includes("@") ) {
                    // Determine Category
                    // note: this is how str.split('@') works
                    // "john@smu.edu.sg" -> ['john', 'smu.edu.sg']
        ...
        send_party_invitation() {
            // note the following:
            // join(", ") converts array to string format where each array element is concat with ", "
            // ['a', 'b']  => 'a, b'
            let str = "Invitation sent to " + this.party_email_addresses.join(", ")

            // note: use of backtick and ${}   
            // note: this also works -> let str = `Invitation sent to ${this.party_email_addresses.join(", ")}`
